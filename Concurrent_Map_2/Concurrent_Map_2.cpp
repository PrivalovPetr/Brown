...
public:
	...
	explicit ConcurrentMap(size_t bucket_count) {
		b_count = bucket_count;
		data = vector<MapType>(bucket_count);
		map_mutex = vector<mutex>(bucket_count);
	}

	WriteAccess operator[](const K& key) {
		int rest = hasher(key) % b_count;
		return WriteAccess{ lock_guard<mutex>(map_mutex[rest]), data[rest][key] };
	}
	ReadAccess At(const K& key) const {
		...
		int rest = hasher(key) % b_count;
		return ReadAccess{ lock_guard<mutex>(map_mutex[rest]), data.at(rest).at(key) };
	}

	bool Has(const K& key) const {
		int rest = hasher(key) % b_count;
		auto g = lock_guard<mutex>(map_mutex[rest]);
		if (data[rest].count(key) > 0)
			return true;
		return false;
	}
	...
	MapType BuildOrdinaryMap() const {
		...
		for (int i = 0; i < b_count; ++i) {
			lock_guard<mutex> tmp(map_mutex[i]);
...
private:
	...
	size_t b_count;
	vector<MapType> data;
	mutable vector<mutex> map_mutex;
