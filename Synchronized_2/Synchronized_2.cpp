...
public:
	...
	struct Access {
		lock_guard<mutex> g;
		T& ref_to_value;
		explicit Access(Synchronized& sync) :
			g(sync.value_mutex),
			ref_to_value(sync.value)
		{
		}

		Access(Access& other) {
			ref_to_value = other.ref_to_value;
			g = move(other.g);
		}
	};
	...
	struct ConstAccess {
		...
		explicit ConstAccess(const Synchronized& sync) :
			g(sync.value_mutex),
			ref_to_value(sync.value)
		{
		}

		ConstAccess(ConstAccess& other) {
			ref_to_value = other.ref_to_value;
			g = move(other.g);
		}
	};
