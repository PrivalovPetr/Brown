...
public:
	...
	private:
		...
		size_t degree;
	public:
		explicit Appender(size_t degree_, Polynomial& _p) :
			coeffs_(_p.coeffs_),
			poly_(_p),
			degree(degree_)
		{			
		...
		operator T() const {
			return std::as_const(poly_)[degree];
		}
	
		T operator =(T desired) {
			const std::vector<T>& v(coeffs_);
			...
			if (desired == 0 && degree == coeffs_.size() - 1) {
				coeffs_[coeffs_.size() - 1] = 0;
				poly_.Shrink();
			}
			
			coeffs_[degree] = desired;
			return desired;
		}
	};
	...
	Appender operator [](size_t degree) {
		T value = degree < coeffs_.size() ? coeffs_[degree] : 0;
		return Appender(degree, *this);
	}
