...
namespace ParseXML_NS {
	...
	void GoThroughTree(Node n, vector<Spending>& res) {
		if (n.Name() == "spend") {
			res.push_back({ n.AttributeValue<string>("category"), n.AttributeValue<int>("amount") });
			return;
		}
		for (Node node : n.Children()) {
			GoThroughTree(node, res);
		}
	}
